  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-8">
      ...
      <div className="flex items-start gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-display gold-gradient-text font-bold">{meta.name}</h1>
          <span className="text-base font-arabic text-gold-dim/60">{meta.nameArabic}</span>
      ...
      <div className="mb-8">
        <details className="group rounded-xl border border-gold-dim/15 bg-card/40 overflow-hidden">
          <summary className="flex items-center gap-2 p-4 cursor-pointer text-sm font-medium text-foreground hover:text-gold-light transition-colors">
            <BookMarked className="size-4 text-gold-light" />
          ...
          <div className="border-t border-gold-dim/10 p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {books.map((book) => (
                <Link
                  key={book.id}
                  href={`/hadith/${collection}/${book.id}`}
                  className="group/book flex items-center justify-between rounded-lg border border-border/20 bg-space-mid/20 p-3 transition-all hover:border-gold-dim/30 hover:bg-gold-dim/5"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="flex size-6 shrink-0 items-center justify-center rounded bg-gold-dim/10 text-[10px] font-medium text-gold-light">
                      {book.id}
                    </span>
                    <span className="text-xs text-foreground truncate group-hover/book:text-gold-light transition-colors">{book.name}</span>
                  </div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-4">
          <Search className="size-4 text-gold-light" />
